...
if [ -z "$VALID" ]; then
  ...
  while [ -z "${STORAGE:+x}" ]; do
    STORAGE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --title "Storage Pools" --radiolist \
      "Which storage pool would you like to use for the immortalwrt VM?\n\n" \
...
FILE="${FILE%.*}"
msg_ok "Extracted immortalwrt Disk Image ${CL}${BL}$FILE${CL}"
...
<div align='center'>
  ...
  <h2 style='font-size: 24px; margin: 20px 0;'>immortalwrt VM</h2>
...
qm start $VMID
sleep 15
msg_info "Waiting for immortalwrt to boot..."
for i in {1..30}; do
  if qm status "$VMID" | grep -q "running"; then
    sleep 5
    msg_ok "immortalwrt is running"
    break
  fi
  sleep 1
done
...
if qm status "$VMID" | grep -q "running"; then
  ...
  msg_ok "Network interfaces configured in immortalwrt"
else
  msg_error "VM is not running"
  exit 1
fi

msg_info "Waiting for immortalwrt to shut down..."
until qm status "$VMID" | grep -q "stopped"; do
  sleep 2
done
msg_ok "immortalwrt has shut down"
